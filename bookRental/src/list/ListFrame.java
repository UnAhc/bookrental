...
public class ListFrame extends JFrame {
	...
	private void setCenterPanel() {
		...
		b.addActionListener(e->{
			...
			ListData listData = new ListData();
			String[][] dataList = listData.loadData();
			tm.setRowCount(0);
			b2.setEnabled(false);

			for(String[] data : dataList) {
				// 책제목 검색창애 넣은 단어가 있는지를 확인후 없으면 데이블에 추가 하지 않는다
...
//				System.out.println("column indec값은" + col);
//				System.out.println(dataList[row][col]);
//				System.out.println(t.getValueAt(row, col));
				for(int i = 0; i < t.getColumnCount(); i++) {
//					System.out.print(t.getModel().getValueAt(row, i));
					ListData listData = new ListData();
					String[][] dataList = listData.loadData();
					dataList = new String[row][i];
				}
			}
		});
	}
